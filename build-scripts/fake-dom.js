...
FakeDomNode.prototype.appendChild = function (child) {
    if(!child) throw "You cannot append a falsey value!"
...
/**
 * Search and retrieve an array of nodes with any of the specified tag names
 * @param {string[]} tagNames Tag names to retrieve elements by
 * @returns {FakeDomNode[]} Array of nodes matching the tag name
 */
FakeDomNode.prototype.getElementsByTagNames = function(tagNames) {
    let children = this.childNodes.filter(node => {
        return tagNames.includes(node.nodeName);
    });

    this.childNodes.forEach(node => {
        children = children.concat(node.getElementsByTagName(tagNames));
    });
