...
Pops a number, `offset`, off of the stack; skips `offset` **instructions**. `offset` may be positive or negative. If `offset` is 0, this has the same effect as `pop`: no jumping.
...
Pops a value and a number off of the stack; if that value is **truthy**, skips `offset` instructions. `offset` may be positive or negative.  If `offset` is 0, this has the same effect as `pop`: no jumping.
